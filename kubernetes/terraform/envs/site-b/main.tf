terraform {
  ...
  backend "s3" {
    ...
    key            = "kubernetes/site-b/terraform.tfstate"
    region         = "eu-west-1"
...
module "site" {
  ...
  metallb_addresses    = ["198.51.100.40-198.51.100.60"]
  cluster_mesh_domain  = "mesh.gitops.lan"
  nodes = [
    {
      name       = "site-b-control-1"
      role       = "control-plane"
      ip_address = "10.20.0.10"
      public_ip  = "198.18.20.10"
    },
    {
      name       = "site-b-worker-1"
      role       = "worker"
      ip_address = "10.20.0.11"
      public_ip  = "198.18.20.11"
    },
    {
      name       = "site-b-worker-2"
      role       = "worker"
      ip_address = "10.20.0.12"
      public_ip  = "198.18.20.12"
    }
  ]
  wireguard_peers = [
    {
      peer_name   = "site-a"
      endpoint    = "public.site-a.example.com:51820"
      public_key  = "REPLACE_ME_WITH_PEER_KEY"
      allowed_ips = ["10.44.0.0/16", "203.0.113.10/29"]
    }
  ]
}
