...
- `lenovo-m710q`: ThinkCentre M710q Tiny with NVMe + 2.5" SATA. The profile
  keeps the NVMe as the system disk and exposes the SATA SSD separately,
  mounted independently under `/mnt/disk1`.
...
- `additional_disk_devices`: extra disks formatted and mounted individually at
  `/mnt/diskX` (for example `['/dev/sda']`).
